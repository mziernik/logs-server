...
function DsTreeNode(root, parent, id, name) {
    ...
    this.isExpandable = () => {
        ...
        if (this.marker)
            this.marker.cls("ds-tree-marker " + (result ? "fa fa-angle-right" : ""));
    ...
    this.expand = (expanded) => {
        ...
        if (this.parent)
            $(this.ul).slideToggle(200, () => {
                if (this.marker)
                    this.marker.cls("ds-tree-marker fa fa-angle-"
    ...
    this.update = () => {
        ...
        this.marker = hdr.tag("span").cls("ds-tree-marker");

        hdr.title = this.title ? this.title : name + (this.comment ? " (" + this.comment + ")" : "");

        hdr.tag("span", this.name).cls("ds-tree-name");
        if (this.comment)
            hdr.tag("span", this.comment).cls("ds-tree-comment");
